...
export function WorkspaceList({ workspaces: initialWorkspaces, organizations, defaultOrganizationId }: { workspaces: Workspace[]; organizations: Organization[]; defaultOrganizationId:string }) {
  ...
  return <div className="workspace-manager">
    ...
    {organizations.length>0&&<form className="create-box" onSubmit={create}><div className="create-box-head"><div><h2>Nuovo workspace</h2><p>La tua organizzazione predefinita è già selezionata. Puoi usare un’altra organizzazione solo se la amministri.</p></div><span className="pill">NUOVO</span></div><div className="create-workspace-grid"><label>Organizzazione<select name="organizationId" required defaultValue={defaultOrganizationId}>{organizations.map(org=><option value={org.id} key={org.id}>{org.name}{org.isDefault?" · PREDEFINITA":""} · {org.plan}</option>)}</select></label><label>Nome<input name="name" placeholder="Es. Progetto cliente" required maxLength={100}/></label><label>Preset<select name="presetKey" defaultValue="GENERAL">{presets.map(([key,label])=><option value={key} key={key}>{label}</option>)}</select></label><label>Lingua<select name="locale" defaultValue={organizations.find(org=>org.id===defaultOrganizationId)?.locale||"it"}>{languages.map(([key,label])=><option value={key} key={key}>{label}</option>)}</select></label></div><button className="btn accent" disabled={busy==="create"}>{busy==="create"?"Creo…":"Crea workspace"}</button></form>}
  </div>;
}
