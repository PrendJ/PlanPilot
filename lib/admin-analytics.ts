...
export async function getAdminAnalytics(period = currentPeriodKey()) {
  ...
  const organizationRows = organizations.map((organization) => {
    ...
    return { id: organization.id, name: organization.name, slug: organization.slug, legalType: organization.legalType, plan: organization.plan, licenseSource: organization.licenseSource, accessExpiresAt: organization.accessExpiresAt, readOnly: Boolean(organization.readOnlyAt), subscriptionStatus: organization.subscription?.status || null, members: organization._count.members, workspaces: organization._count.workspaces, createdBy: organization.createdBy, revenueEur, aiCostUsd: usage.cost, aiCostEur, stripeFeesEur, contributionEur: revenueEur - aiCostEur - stripeFeesEur, aiRequests: usage.requests };
  });
  ...
  return {
    ...
    users: users.map((user) => { const usage = userUsage.get(user.id) || { cost: 0, requests: 0 }; return { id: user.id, name: user.name, email: user.email, isAdmin: user.isAdmin, canCreateWorkspaces: user.canCreateWorkspaces, lifetimeFree: user.lifetimeFree, emailVerified: Boolean(user.emailVerifiedAt), organizations: user._count.organizationMemberships, workspaces: user._count.memberships, aiRequests: usage.requests, aiCostUsd: usage.cost, aiCostEur: usage.cost * usdToEur }; }),
