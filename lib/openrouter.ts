...
export async function planPatchFromText(input: { apiKey: string; model: string; workspaceName: string; userText: string; plan: unknown }) {
  ...
  const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    headers: {
      ...
      "HTTP-Referer": process.env.APP_URL || "https://voxboard.draftapps.it",
      "X-OpenRouter-Title": "VoxBoard AI",
    },
    body: JSON.stringify({
      model: input.model,
      messages: [
        { role: "system", content: system },
        { role: "user", content: `Workspace: ${input.workspaceName}\n\nCURRENT PLAN JSON:\n${JSON.stringify(input.plan)}\n\nUSER UPDATE:\n${input.userText}` },
      ],
