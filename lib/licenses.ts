...
import { PLANS, planKey, TRIAL_DAYS, type PlanKey } from "@/lib/plans";
...
export async function grantManualLicense(input: {
  ...
  const trialEndsAt = input.plan === "TRIAL" ? expiresAt || new Date(Date.now() + TRIAL_DAYS * 86400000) : null;
  return prisma.$transaction(async tx => {
    const stored = await tx.organization.findUniqueOrThrow({
      ...
      select: {
        ...
        createdBy: { select: { lifetimeFree: true } },
      },
    });
    ...
    const organization = await tx.organization.update({
      ...
      data: {
        plan: input.plan,
        licenseSource: input.plan === "LIFETIME" ? "LIFETIME" : "MANUAL",
    ...
    await tx.adminAuditEvent.create({
      data: {
        ...
        metadata: { before, plan: input.plan, expiresAt, reason: input.reason },
      },
    });
    return organization;
  });
...
export async function revokeManualLicense(input: { organizationId: string; actorId: string; reason: string }) {
  return prisma.$transaction(async tx => {
    ...
    const plan = subscription?.plan || planFromPrice(price)?.plan || "TRIAL";
    const organization = await tx.organization.update({
      where: { id: input.organizationId },
      data: active
        ? { plan, licenseSource: "STRIPE", accessExpiresAt: null, trialEndsAt: null, readOnlyAt: null, deleteAfter: null }
        : {
            plan: "TRIAL",
            licenseSource: "TRIAL",
    ...
    await tx.adminAuditEvent.create({
      data: {
        actorId: input.actorId,
        action: "MANUAL_LICENSE_REVOKED",
        targetType: "ORGANIZATION",
        targetId: input.organizationId,
        metadata: { reason: input.reason, fallback: active ? plan : "TRIAL" },
      },
    });
    return organization;
  });
