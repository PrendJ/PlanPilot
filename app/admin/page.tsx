...
export default async function AdminPage(){
  ...
  const [users,workspaces,analytics]=await Promise.all([prisma.user.findMany({orderBy:[{name:"asc"},{email:"asc"}],select:{id:true,email:true,name:true,platformRole:true,lifecycleStatus:true,isAdmin:true,lifetimeFree:true,emailVerifiedAt:true,memberships:{select:{role:true,workspace:{select:{id:true,name:true,slug:true}}}},organizationMemberships:{select:{role:true,organization:{select:{id:true,name:true,slug:true,plan:true,licenseSource:true,lifecycleStatus:true}}}}}}),prisma.workspace.findMany({orderBy:{name:"asc"},select:{id:true,name:true,slug:true,lifecycleStatus:true,organization:{select:{name:true,plan:true,licenseSource:true,lifecycleStatus:true}}}}),getAdminAnalytics()]);
