...
async function requireAdmin() { const user = await getCurrentUser(); return user?.isAdmin ? user : null; }
const createSchema = z.object({ name: z.string().trim().min(2).max(100), email: z.string().email().max(254), password: z.string().min(10).max(200), isAdmin: z.boolean().default(false), canCreateWorkspaces: z.boolean().default(false), lifetimeFree: z.boolean().default(false) });
const updateSchema = z.object({ id: z.string().cuid(), isAdmin: z.boolean().optional(), canCreateWorkspaces: z.boolean().optional(), lifetimeFree: z.boolean().optional(), emailVerified: z.boolean().optional() });

const userSelect = { id: true, email: true, name: true, isAdmin: true, canCreateWorkspaces: true, lifetimeFree: true, emailVerifiedAt: true, memberships: { select: { role: true, workspace: { select: { id: true, name: true, slug: true } } } }, organizationMemberships: { select: { role: true, organization: { select: { id: true, name: true, slug: true, plan: true } } } } } as const;
...
export async function POST(request: Request) {
  ...
  try {
    const user = await prisma.user.create({ data: { email: body.email.toLowerCase(), name: body.name, passwordHash: await bcrypt.hash(body.password, 12), emailVerifiedAt: new Date(), isAdmin: body.isAdmin, canCreateWorkspaces: body.canCreateWorkspaces || body.isAdmin, lifetimeFree: body.lifetimeFree, lifetimeFreeGrantedAt: body.lifetimeFree ? new Date() : null, lifetimeFreeGrantedBy: body.lifetimeFree ? actor.id : null }, select: userSelect });
    await createOrganization({ name: `${body.name}`, userId: user.id, locale: "it", legalType: "PERSONAL" });
    await prisma.adminAuditEvent.create({ data: { actorId: actor.id, action: "USER_CREATED", targetType: "USER", targetId: user.id, metadata: { lifetimeFree: body.lifetimeFree, isAdmin: body.isAdmin } } });
...
export async function PATCH(request: Request) {
  ...
  const body = parsed.data;
  if (body.id === actor.id && body.isAdmin === false) return NextResponse.json({ error: "Non puoi rimuovere il tuo ruolo superadmin" }, { status: 400 });
  const before = await prisma.user.findUnique({ where: { id: body.id }, select: { isAdmin: true, canCreateWorkspaces: true, lifetimeFree: true, emailVerifiedAt: true } }); if (!before) return NextResponse.json({ error: "Utente non trovato" }, { status: 404 });
