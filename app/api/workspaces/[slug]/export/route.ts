...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const plan = { workspace: workspace.name, exportedAt: new Date().toISOString(), columns: workspace.columns.map((c) => ({ title: c.title, kind: c.kind, cards: c.cards.map((x) => ({ title: x.title, description: x.description, priority: x.priority, dueDate: x.dueDate, tags: x.tags })) })) };
  if (format === "md") {
