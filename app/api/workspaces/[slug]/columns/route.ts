...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try { const result = await prisma.$transaction(async (tx) => { if (!(await assertRevision(workspace.id, parsed.data.revision, tx, { userId: user.id, manageColumns: true }))) throw new Error("STALE_REVISION"); const position = await tx.boardColumn.count({ where: { workspaceId: workspace.id } }); const column = await tx.boardColumn.create({ data: { workspaceId: workspace.id, title: parsed.data.title, description: parsed.data.description, semanticKey: "CUSTOM", position } }); await logActivity(tx, { organizationId: workspace.organizationId, workspaceId: workspace.id, userId: user.id, type: "COLUMN_CREATED", entityType: "COLUMN", entityId: column.id, afterState: column as never }); const revision = (await bumpRevision(tx, workspace.id)).revision; return { column, revision }; }); return NextResponse.json(result, { status: 201 }); } catch (error) { if (isBoardConflict(error)) return NextResponse.json({ error: "Board changed. Reload and retry." }, { status: 409 }); throw error; }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try { const revision = await prisma.$transaction(async (tx) => { if (!(await assertRevision(workspace.id, parsed.data.revision, tx, { userId: user.id, manageColumns: true }))) throw new Error("STALE_REVISION"); for (const [position, id] of parsed.data.columnIds.entries()) await tx.boardColumn.update({ where: { id }, data: { position } }); await logActivity(tx, { organizationId: workspace.organizationId, workspaceId: workspace.id, userId: user.id, type: "COLUMNS_REORDERED", entityType: "COLUMN", afterState: parsed.data.columnIds as never }); return (await bumpRevision(tx, workspace.id)).revision; }); return NextResponse.json({ revision }); } catch (error) { if (isBoardConflict(error)) return NextResponse.json({ error: "Board changed. Reload and retry." }, { status: 409 }); throw error; }
}
