...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try { const result = await prisma.$transaction(async (tx) => { if (!(await assertRevision(workspace.id, body.revision, tx, { userId: user.id }))) throw new Error("STALE_REVISION"); const position = await tx.card.count({ where: { columnId: body.columnId, archived: false } }); const card = await tx.card.create({ data: { workspaceId: workspace.id, columnId: body.columnId, title: body.title, description: body.description, priority: body.priority, dueDate: body.dueDate ? new Date(body.dueDate) : null, tags: body.tags, position, assignees: { create: body.assigneeIds.map((userId) => ({ userId })) } }, include: { assignees: { include: { user: { select: { id: true, name: true, email: true } } } } } }); await logActivity(tx, { organizationId: workspace.organizationId, workspaceId: workspace.id, userId: user.id, type: "CARD_CREATED", entityType: "CARD", entityId: card.id, afterState: card as never }); const revision = (await bumpRevision(tx, workspace.id)).revision; return { card, revision }; }); return NextResponse.json(result, { status: 201 }); } catch (error) { if (isBoardConflict(error)) return NextResponse.json({ error: "Board changed. Reload and retry." }, { status: 409 }); throw error; }
}
