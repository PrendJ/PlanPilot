...
export async function POST(request: Request) {
  ...
  const limited = await rateLimit(`verify-resend:${clientIp(request)}`, 5, 60 * 60_000, request);
  ...
  const delivery = user
    ? user.emailVerifiedAt
      ? await issueExistingAccountEmail(user, request, parsed.data.next)
      : await issueVerificationEmail(user, request, parsed.data.next)
    : "sent";
