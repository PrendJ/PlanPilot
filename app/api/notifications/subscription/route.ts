...
export async function GET() {
  ...
  const subscribed = config ? Boolean(await prisma.pushSubscription.findFirst({ where: { sessionId: session.id }, select: { id: true } })) : false;
  return NextResponse.json({ enabled: Boolean(config), publicKey: config?.publicKey || null, subscribed }, { headers: { "Cache-Control": "no-store" } });
}

export async function POST(request: Request) {
  const origin = rejectCrossOrigin(request); if (origin) return origin;
  const session = await getCurrentSession();
  ...
  const limited = rateLimit(`push:${session.id}`, 10, 60000); if (limited) return limited;
  ...
  try {
    await prisma.$transaction(async tx => {
      ...
      const existing = await tx.pushSubscription.findFirst({ where: { endpoint: parsed.data.endpoint, session: { userId: session.userId } } });
      if (existing?.sessionId === session.id && existing.p256dh === parsed.data.keys.p256dh && existing.auth === parsed.data.keys.auth) return;
